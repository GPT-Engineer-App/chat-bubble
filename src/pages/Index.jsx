    ...
    <ChakraProvider>
      <Container maxW="container.md" p={4}>
        <VStack spacing={4}>
          ...
          <Flex flexDirection="column" p={4} bg="gray.100" borderRadius="lg" flexGrow={1} overflowY="auto">
            <Stack spacing={4}>
              {messages.map((message, index) => (
                <Flex key={index} bg="blue.100" p={3} borderRadius="lg" alignSelf="flex-end" maxWidth="80%">
                  <Text>{message}</Text>
                </Flex>
              ))}
            </Stack>
          ...
          <InputGroup size="md">
            <Input pr="4.5rem" value={inputValue} onChange={handleInputChange} onKeyPress={handleKeyPress} placeholder="Type a message..." />
